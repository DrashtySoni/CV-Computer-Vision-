...
# SciPy uses NumPy arrays as the basic data structure, and comes with modules for various commonly
# used tasks in scientific programming, including linear algebra, integration (calculus),
# ordinary differential equation solving, and signal processing.
from scipy.spatial import distance as dist
import math

# input video path
i_vid = r'C:/Users/Rushi/Desktop/PySimpleGUI-YOLO-ComputerVision/np.mp4'
# output video path
o_vid = r'C:/Users/Rushi/Desktop/PVs/new.avi'
# yolo dataset path
y_path = r'yolo-coco'
...
###################################   UI LAYOUT   #############################################
layout = [
    # [sg.Text('DRASHTY-UNNATI'S VIDEO PROCESSING TOOL', size=(52, 2), font=('Any', 18), text_color='#8B0000', justification='left')],
...
win = sg.Window('DRASHTY-UNNATI VIDEO PROCESSING TOOL',
                default_element_size=(25, 1),
                text_justification='right',
                auto_size_text=False,
                ).Layout([[sg.Column(layout=layout,size=(1920,1080))]])
...
if event is None or event == 'OK':
    EnableCar = True
    EnableCarBackup = True
    EnableTruck = True
    EnableTruckBackup = True
    EnableBus = True
    EnableAnimal = True
    EnablePerson = True
    ...
    CarColor = (0, 0, 0)
    CarWheelOutColor = (0, 0, 0)
    CarWheelInColor = (255, 255, 255)

    TruckSize = 3
    newT = 3
    TruckColor = (0, 0, 0)
    TruckWheelOutColor = (0, 0, 0)
    TruckWheelInColor = (255, 255, 255)
...
while True:
    ...
    if len(idxs) > 0:
        ...
        for i in idxs.flatten():
            ...
            # coordinates for default boundary box
            # 1 2
            # 3 4
            c5 = (x, y)  # 1
            c6 = (x + w, y)  # 2
            ...
            ######################################   ICON COORDINATES   ####################################            # REF POINT FOR ALL ICON DRAWINGS
            # iconPositionx = int(20)
            # iconPositiony = int(50)
...
###########################################  DETECTION END  #################################################
            ...
            if LABELS[classIDs[i]] == 'truck' and EnableTruck:
    ...
    if not win_started:
        ...
        layout = [
            # [sg.Image("/Users/Drashty/Desktop/PySimpleGUI-YOLO-ComputerVision/images/logo.png", key="imageContainer")]
            [sg.Text('Drashty-Unnati', size=(30, 1))],
        ...
        win = sg.Window('YOLO Output',
                        ...
                        resizable=True,
                        text_justification='right',
                        auto_size_text=False).Layout([[sg.Column(layout=layout,size=(1920,1080), scrollable=True)]]).Finalize()
